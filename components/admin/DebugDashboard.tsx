...
const DebugDashboard: React.FC<DebugDashboardProps> = ({ navigation }) => {
  ...
  const executeQuery = async () => {
    ...
    try {
      ...
      Alert.alert('Query Error', error instanceof Error ? error.message : 'Query execution failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportDebugData = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Debug data exported to console. In production, this would download a file.');
  ...
  const renderTabButton = (tab: string, label: string, icon: string) => (
    ...
    >
      <Ionicons name={icon as any} size={20} color={activeTab === tab ? '#fff' : '#666'} />
    ...
    <View style={styles.section}>
      ...
      {systemInfo && (
        <View style={styles.card}>
          ...
          <Text style={styles.infoText}>IndexedDB Support: {systemInfo.indexedDBSupport ? 'Yes' : 'No'}</Text>
          <Text style={styles.infoText}>Databases: {systemInfo.databases.join(', ')}</Text>
          <Text style={styles.infoText}>Timestamp: {new Date(systemInfo.timestamp).toLocaleString()}</Text>
        </View>
      )}
      ...
      {databaseStats && (
        <View style={styles.card}>
          ...
          <Text style={styles.infoText}>Total Users: {databaseStats.totalUsers}</Text>
          <Text style={styles.infoText}>Total Businesses: {databaseStats.totalBusinesses}</Text>
          <Text style={styles.infoText}>Total Reviews: {databaseStats.totalReviews}</Text>
          
          <Text style={styles.subTitle}>Users by Type:</Text>
          {Object.entries(databaseStats.usersByType).map(([type, count]) => (
            <Text key={type} style={styles.infoText}>  {type}: {count}</Text>
          ))}
          
          <Text style={styles.subTitle}>Businesses by Category:</Text>
          {Object.entries(databaseStats.businessesByCategory).map(([category, count]) => (
            <Text key={category} style={styles.infoText}>  {category}: {count}</Text>
          ))}
        </View>
      )}
    </View>
    ...
    <View style={styles.section}>
      ...
      <View style={styles.logFilters}>
        <View style={styles.filterRow}>
          ...
          {['all', 'error', 'warn', 'info', 'debug'].map(level => (
            <TouchableOpacity
              key={level}
              style={[styles.filterChip, logLevel === level && styles.activeFilterChip]}
              onPress={() => setLogLevel(level)}
            >
              <Text style={[styles.filterChipText, logLevel === level && styles.activeFilterChipText]}>
                {level}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <ScrollView style={styles.logsContainer}>
        {getFilteredLogs().map(log => (
          <View key={log.id} style={[styles.logEntry, styles[`log${log.level.charAt(0).toUpperCase() + log.level.slice(1)}`]]}>
  ...
  const renderDatabase = () => (
    <View style={styles.section}>
      ...
      <TouchableOpacity style={styles.actionButton} onPress={() => setShowQueryModal(true)}>
      ...
      {databaseStats && (
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Recent Businesses ({databaseStats.recentBusinesses.length})</Text>
    ...
    <View style={styles.section}>
      ...
      {performanceResults && (
        <View style={styles.card}>
          ...
          <Text style={styles.infoText}>Database Read: {performanceResults.dbReadTime.toFixed(2)}ms</Text>
          <Text style={styles.infoText}>Database Write: {performanceResults.dbWriteTime.toFixed(2)}ms</Text>
          <Text style={styles.infoText}>Auth Check: {performanceResults.authTime.toFixed(2)}ms</Text>
          <Text style={styles.infoText}>Tested: {new Date(performanceResults.timestamp).toLocaleString()}</Text>
        </View>
      )}
    </View>
  );
  ...
  const renderTools = () => (
    <View style={styles.section}>
      ...
      <TouchableOpacity style={[styles.actionButton, styles.warningButton]} onPress={importSampleData}>
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Query Modal */}
      <Modal visible={showQueryModal} animationType="slide" presentationStyle="pageSheet">

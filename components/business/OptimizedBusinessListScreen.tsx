...
export default function OptimizedBusinessListScreen({ navigation }: BusinessListScreenProps) {
  ...
  const renderBusinessCard = ({ item }: { item: any }) => (
    <TouchableOpacity 
      ...
      onPress={() => navigation.navigate('BusinessDetails', { businessId: item.businessId || item.id })}
    >
  ...
  const renderFeaturedSection = () => {
    if (!featuredBusinesses.length || featuredLoading) return null;
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {featuredBusinesses.slice(0, 5).map((business) => (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('BusinessDetails', { businessId: business.businessId })}
            >
              <Text style={styles.featuredName}>{business.name}</Text>
              <Text style={styles.featuredCategory}>{business.category}</Text>
